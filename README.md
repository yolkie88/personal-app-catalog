...
个人应用清单中心，附带 Windows、WSL 和 macOS 恢复脚本。

这个仓库维护的是“长期认可、愿意跨设备恢复的应用目录”，不是某台电脑的软件盘点。能自动安装的 Windows 内容放在 `windows/manifests/`，WSL 开发环境放在 `wsl/`，macOS 恢复面放在 `mac/`，不能或不应该自动恢复的内容写入文档的手工边界。
...
macOS 侧以 Homebrew/Brewfile 为主，mise 管开发运行时，当前按 Mac mini 长期开机、家庭核心管理枢纽设计。先预览：
...
`home-hub` 只安装 Tailscale、Syncthing、Kopia/restic/rclone、Caddy、sing-box/Mihomo、网络诊断和远控等工具，不自动启用服务；真实服务配置、代理订阅、证书、备份密钥和设备授权见 `mac/docs/home-hub.md` 与 `mac/docs/manual-boundaries.md`。
...
| `mac/docs/catalog.md` | macOS profile、Homebrew 清单和 Mac mini 家庭枢纽总览 |
| `mac/docs/apps.md` | macOS 每个工具的用途、保留理由和恢复边界 |
| `mac/docs/config.md` | macOS 工具配置层和 `configure.sh` |
| `mac/docs/home-hub.md` | Mac mini 长期开机家庭枢纽服务边界 |
| `mac/docs/manual-boundaries.md` | macOS 账号、授权、代理、备份、服务和 App Store 手工边界 |
...
- macOS 侧默认层保持小；`home-hub` 面向长期开机 Mac mini，服务只安装工具不自动启用。
...
- 提交前运行 `windows/validate.ps1`、`bash wsl/validate.sh` 和 `bash mac/validate.sh`；恢复前优先使用 `-Plan` / `--plan` 确认将要执行的安装。
