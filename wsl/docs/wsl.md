...
> 前置顺序：先配好 mirrored 网络（`.\windows\configure.ps1 -Wsl` + `wsl --shutdown`），进入 WSL 后先跑 `./wsl/bootstrap.sh --proxy` 配常开代理，最后才装工具链。否则受限网络下 `--base`、Docker、mise 和 agentic CLI 可能装不上。完整顺序见 `windows/docs/recovery-playbook.md` 第 5 节和 `wsl/docs/proxy.md`。
...
Docker daemon 代理属于本机运行期配置，由显式 `./wsl/bootstrap.sh --proxy` 写入。需要代理 `docker pull` / build 时，先跑 `--proxy`，再安装或重启 Docker。
