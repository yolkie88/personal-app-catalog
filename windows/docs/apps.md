...
| 工具 | 用途 | 保留理由 | 恢复边界 |
|---|---|---|---|
| sing-box | 默认代理核心 | 当前默认用户态系统代理核心，配合 Windows 系统代理和 WSL mirrored 使用 | 订阅、节点、secret、运行目录和自启动方式按设备处理 |
| Mihomo | 备用代理核心 | 备用核心，适合已有 mihomo 规则或 WinSW 服务化场景 | 订阅、节点、secret 和真实配置不入库 |
| WinSW | Windows 服务封装 | 将 mihomo 封装为 Windows 服务，开机自启 | 服务路径、账号和权限按设备处理 |

> sing-box、Mihomo 和 WinSW 是 winget 的 archive / portable 包：只解压 exe 到 `%LOCALAPPDATA%\Microsoft\WinGet\Packages\…` 的哈希目录里，无安装器、无 PATH shim、不好找。装完用 `.\windows\publish-tools.ps1` 把它们落到 `C:\Tools\...` 这种固定路径，给启动器和服务/计划任务一个稳定引用。winget 仍是版本来源，`winget upgrade` 后重跑一次 publish 即可。详见 `windows/docs/proxy.md`。
