...
Separate from the *package* manifests, sanitized tool-config templates live in `windows/config/` and `wsl/config/` (PowerShell profile + modules, Windows Terminal defaults, shared Git config, VS Code extensions + settings, Neovim/lazy.nvim, starship, tmux, bat, lazygit, bash aliases). They are applied **opt-in** and **plan-first** by `windows/configure.ps1` (`-Pwsh`/`-Terminal`/`-Git`/`-VSCode`/`-All`, `-Plan`) and `wsl/bootstrap.sh --config`, both of which **back up any existing target before overwriting** and are idempotent (guarded `$PROFILE`/`.bashrc` insertion via a `personal-app-catalog` marker; Git config layered via `include.path`). The VS Code layer manages only recommended extensions and sanitized settings defaults (deep-merged) — never account/Settings-Sync keys or private feeds.

Key rules: the config layer is **not** a winget profile — never add it to `bootstrap.ps1`'s `ValidateSet`, the `all` set, or the `catalog.md` profile tables. Templates must stay **template-only**: no identity (`user.name`/`user.email`), keys, credentials, tokens, or history — both validators scan `*/config/` and fail on secret-like assignments or email strings. `windows/configure.ps1 -Plan` must remain side-effect-free (no external commands) so CI can run it on Linux `pwsh`. delta-dependent Git settings live in a separate `gitconfig.delta` template that the appliers wire via `include.path` **only when `delta` is installed** (the base `gitconfig.shared` has no external dependency); `configure.ps1` also prints an install hint for any missing optional tool (`delta`/`fzf`/`starship`/`lazygit`/`bat`). See `windows/docs/config.md` and `wsl/docs/config.md`.
...
- **Secrets and machine state never enter Git.** `.gitignore` blocks tokens, keys, subscriptions, exports/reports, and runtime config. Things that can't or shouldn't be auto-restored (licenses, hardware, accounts) are documented as "manual boundaries" in `windows/docs/manual-boundaries.md` and `wsl/docs/wsl-boundaries.md` rather than scripted. Note the `**/config.yml` runtime-config rule also matches `wsl/config/lazygit/config.yml`, so that template is explicitly re-allowed with a `!wsl/config/lazygit/config.yml` negation — any future `config.yml`/`config.yaml` template under `*/config/` needs the same negation, or it will be silently skipped by commits.
...
`windows/docs/` covers the Windows catalog (`catalog.md`), per-app rationale (`apps.md`), source priority (`sources.md`), manual boundaries, operations, and the new-device `recovery-playbook.md`. `wsl/docs/` covers the WSL-first environment (`wsl.md`), tools (`tools.md`), and WSL boundaries. `docs/agent-workflows.md` is the operations handbook for coding agents (task templates, review checklist, permission boundaries).
